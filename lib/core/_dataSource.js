...
const _dataSource = {
  ...
  create: async (dir, file, data, callback) => {
    const buildedRelatedDir = `${_dataSource.BASE_DIR}${dir}`;
    const buildedPath = `${buildedRelatedDir}/${file}.${_dataSource.DEFAULT_FILE_EXTENSION}`;
    const flags = 'wx';

    await helpers.makeDirectoryIfNotExists(buildedRelatedDir);
